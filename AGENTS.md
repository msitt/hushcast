...
plus `skipped` (pre-existing episodes are never auto-processed by design, queue them manually via Process), `failed` (retryable, → queued), `review` (detection failed a sanity check such as the circuit breaker. Segments are stored, nothing is cut, never auto-retried. A human trims and cuts, re-detects, or dismisses), and `expired` (retention cleanup). Promo-only episodes (the LLM sets `whole_episode_promo` and the segments cover most of the episode) end as `skipped` or pass through untouched per the `promo_episode_action` setting. A step ends the pipeline early in one of these statuses by raising `state.StopPipeline`. Whitelisted feeds run a shortened copy-through pipeline (download → cut → finalize, no transcribe/detect). All status changes must go through `validate_transition`.
